...
Arithmetic averaging is straightforward. All subspectra for each editing experiment $x_m$ (e.g., all edit-ON and all edit-OFF subspectra) are averaged using the arithmetic mean: $\bar{x} = \frac{1}{M}\sum{x_m}$. The average difference between the averaged edit-ON and edit-OFF spectra is then calculated to yield the final difference spectrum: $x_{DIFF} = (\bar{x}_{ON} - \bar{x}_{OFF})/2$.
