...
|:------------------|:----------------------------------------------------|
| `loadtime` | The time when `GannetLoad` was initiated |
...
| `mask_overlap` | Structure giving details of where a voxel mask overlap is saved (this is created if the `normalize` flag is set to `1` in `GannetPreInitialise.m`) |
