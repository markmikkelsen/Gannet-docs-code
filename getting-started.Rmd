...
```{r, child = c("js/back-to-top.html", "js/copy-to-clipboard.html", "js/tocify-offset.html")}
```
...
If this is your first time using Gannet, you should read through the [Reference](./reference.html) as well. If you have previously worked with Gannet and only need a quick refresher about the command syntax, this page will probably be all you need to get started again.
...
|:---------------|:-------------------------------------------------------|
| **GannetLoad** | Loads raw MRS data files exported from the scanner; preprocesses them; plots the edited spectra; and gives information about the time course of the experiment. |
...
| **GannetCoRegister** | Loads structural MR images; co-registers an MRS voxel mask to them; and produces a figure showing the localization of the voxel on the structural image on three orthogonal slices. |
| **GannetSegment** | Calls SPM to segment the structural images; determines the fractional tissue composition of gray matter, white matter, and CSF within the MRS voxel; produces a figure showing the tissue-segmented voxel masks as overlays; and displays CSF-corrected metabolite estimates. |
...
The first step in using Gannet is to set up an analysis with the `GannetPreInitialise.m` function. While the toolkit has many routines that automatically recognize data formats, sequences, and acquisition parameters from data file headers; some settings need to be set by the user to run Gannet properly. Additionally, `GannetPreInitialise.m` allows the user to choose how to process their data (e.g., which frequency-and-phase correction algorithm to use).
...
The default settings pre-initialize a standard analysis for in vivo GABA-edited MEGA-PRESS data. If you want to analyze data acquired with a different method or from a phantom, or target a different metabolite, you will need to make changes to this file. Please read the [Reference](./reference.html#pre-initializing-gannet) for detailed explanations of the options and flags in `GannetPreInitialise.m`.
...
::: info
<i class="fa fa-info-circle" style="color: white"></i>  Note that the edit-OFF file must be input first.
:::

### Co-registration to MR images

After MRS data files are loaded and fitted, and if an structural MR image was acquired and used to place MRS voxels, Gannet can co-register the two and create a binary voxel mask in image space. To do so, use the following command:
...
Gannet uses SPM12's segmentation routine to segment structural MR images into grey matter (GM), white matter (WM), and cerebrospinal fluid (CSF) probabilistic tissue maps, from which voxel tissue fractions are then used in subsequent tissue corrections of metabolite concentration estimates.

Installation instructions for SPM12 can be found [here](./index.html#installation).
...
## Compatibility with BIDS

Users are encouraged to organize their MRS data and metadata according to the Brain Imaging Data Structure (BIDS) [@Gorgolewski2016; @Bouchard2025]. Full details about the standard can be found on the <a href="https://bids-specification.readthedocs.io/en/stable/" target="_blank">BIDS website</a>. MRS specifications can be found <a href="https://bids-specification.readthedocs.io/en/stable/modality-specific-files/magnetic-resonance-spectroscopy.html" target="_blank">here</a>.

::: info
<i class="fa fa-info-circle" style="color: white"></i> While Gannet will automatically check if users' BIDS datasets are valid, users may also wish to validate their datasets using the <a href="https://bids-validator.readthedocs.io/en/stable/" target="_blank"><u>BIDS Validator</u></a>, which will perform a more thorough validation check.
:::

### Loading data

The command syntax for loading a BIDS dataset is slightly different. The base directory path name of a BIDS dataset is entered into `GannetLoad.m` as a cell array as the first input argument. The second input argument is a cell array that specifies which MRS data to process. This is the label used with the `acq` entity in the filenames of the single-voxel MRS data files (those ending in `*_svs.nii[.gz]`) to be processed. If the second input argument is not provided, Gannet will process all single-voxel MRS data files in the dataset.

Unsuppressed water reference files are automatically found by parsing the relevant `*_svs.json` files for the `ReferenceSignal` key.

`GannetCoRegister.m` does not require a second input argument for structural image filenames as Gannet will search for the path names of the images associated with each MRS acquisition stored in the `AnatomicalImage` key in the `*_svs.json` files.

This shows that it is crucial that users adhere to the BIDS specifications to avoid running into errors.

Example usage:

```{octave, eval = FALSE}
MRS_struct = GannetLoad('path/to/bids_dataset/', 'megapress');
% 'megapress' would be the 'acq' entity label for the MEGA-PRESS files in
% 'path/to/bids_dataset/'
MRS_struct = GannetFit(MRS_struct);
MRS_struct = GannetCoRegister(MRS_struct);
% No second input argument is needed as Gannet will find the associated
% structural images from the *_svs.json files
MRS_struct = GannetSegment(MRS_struct);
MRS_struct = GannetQuantify(MRS_struct);
```

As shown, only the path name of the BIDS dataset base directory is inputted into `GannetLoad.m`. Gannet will then automatically parse the subject (and session) subdirectories to find all relevant single-voxel MRS data files, including water signal references and structural MR images.
...
% Note that the number of structural images must match the number of MRS
% metabolite data files inputted in GannetLoad
...
We have provided an [example batch processing script](./batch-script.html) that you may modify for your own purposes.
...
::: info
<i class="fa fa-info-circle" style="color: white"></i> The syntax is different for Siemens .rda files as these are typically pre-averaged and, for for MEGA editing, are exported as two files for edit-ON and edit-OFF acquisitions. In such cases, the input cell arrays must be M $\times$ N $\times$ P matrices, where P is the number of subexperiments (e.g., 2 for MEGA-PRESS, 4 for HERMES, etc.).
:::

We have provided an [example script](./joining-data-files.html) for joining files.
...
The output structure contains several fields and subfields, which grow in number as your proceed through the Gannet analysis pipeline. See the list of [output structure attributes](./output-structure-attributes.html) for a complete description.
